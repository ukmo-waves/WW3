...
MODULE W3GDATMD
  ...
  !/
#ifdef W3_GPU
! For CALC_USTART routine:
!$ACC DECLARE CREATE(ZZWND, AALPHA)

! Make some source large lookup tables resident on GPU:
!$ACC DECLARE COPYIN (SSDSBM, SSDSC, DCKI,            &
...
!/CB Added sig, dden, nth, nspec
!$ACC DECLARE COPYIN(KDMN, SDBC1, SDBC2, FDONLY, FSSOURCE, &
!$ACC                   SIG, DDEN, NTH, NK, NSPEC)

!/LS For ROUTINE SEQ SBT1
!$ACC DECLARE COPYIN(MAPWN(:), SBTC1)

!/LS For ROUTINE SEQ LIU_FORWARD_DISPERSION
!$ACC DECLARE COPYIN(IICEHDISP, IICEDDISP, IICEFDISP, IICEHMIN, IICEDISP)

!/LS W3PSMC
!$ACC DECLARE COPYIN(IJKCel, IJKUFc, IJKVFc, NCel, NUFc, NVFc      )&
!$ACC         COPYIN(IJKCel3, IJKCel4,  IJKVFc5, IJKVFc6, CLATS    )&
!$ACC         COPYIN(IJKUFc5,IJKUFc6, NSEA, MRFct, ARCTC, CLATF)

!/LS POST PORT OPTIMISATION
!$ACC DECLARE COPYIN(TRNY, TRNX)
#endif
