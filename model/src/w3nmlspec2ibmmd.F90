...
!  1. Purpose :
!
!     Manages namelists from configuration file ww3_mopp.nml for
!     ww3_spec2ibm program
!
!/ ------------------------------------------------------------------- /
  ...
  CONTAINS
!/ ------------------------------------------------------------------- /
  SUBROUTINE W3NMLSPEC2IBM (NDSI, INFILE, NML_SPEC, NML_FILE, IERR)
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |           M. Accensi              |
...
!      Name      TYPE  Module   Description
!     ----------------------------------------------------------------
!      WW3_SPEC2IBM  Prog.   N/A    Postprocess output fields.
!     ----------------------------------------------------------------
!
!  6. Error messages :
!
...
!/ ------------------------------------------------------------------- /
    ...
    TYPE(NML_SPEC_T), INTENT(INOUT)             :: NML_SPEC
    ...
    IERR = 0
#ifdef W3_S 
    CALL STRACE (IENT, 'W3NMLSPEC2IBM')
#endif
    ...
    ! read field namelist
    CALL READ_SPEC_NML (NDSI, NML_SPEC)
    CALL REPORT_SPEC_NML (NML_SPEC)
  ...
  END SUBROUTINE W3NMLSPEC2IBM


!/ ------------------------------------------------------------------- /
  SUBROUTINE READ_SPEC_NML (NDSI, NML_SPEC)
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |           C. Bunney               |
...
!      Name        TYPE  Module   Description
!     ----------------------------------------------------------------
!    W3NMLSPEC2IBM Subr.   N/A    Namelist configuration routine.
!     ----------------------------------------------------------------
!
!  6. Error messages :
!
...
!/ ------------------------------------------------------------------- /
    ...
    ! locals
    INTEGER                             :: IERR
    TYPE(NML_SPEC_T) :: SPEC
    NAMELIST /SPEC_NML/ SPEC
#ifdef W3_S 
    INTEGER, SAVE                           :: IENT = 0
#endif
    ...
    IERR = 0
#ifdef W3_S 
    CALL STRACE (IENT, 'READ_SPEC_NML')
#endif
    ...
    ! read spec namelist
    REWIND (NDSI)
    READ (NDSI, nml=SPEC_NML, iostat=IERR, iomsg=MSG)
    IF (IERR.NE.0) THEN
      WRITE (NDSE,'(A,/A)') &
        'ERROR: READ_SPEC_NML: namelist read error', &
        'ERROR: '//TRIM(MSG)
      CALL EXTCDE (1)
    END IF
...
!/ ------------------------------------------------------------------- /
  SUBROUTINE REPORT_SPEC_NML (NML_SPEC)
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |           C. Bunney               |
...
!/ ------------------------------------------------------------------- /
    ...
    TYPE(NML_SPEC_T), INTENT(IN) :: NML_SPEC
#ifdef W3_S
    INTEGER, SAVE                           :: IENT = 0

    CALL STRACE (IENT, 'REPORT_SPEC_NML')
#endif

      WRITE (MSG,'(A)') 'SPEC % '
      WRITE (NDSN,'(A)')
      WRITE (NDSN,11) TRIM(MSG),'NK  = ', NML_SPEC%NK
      WRITE (NDSN,11) TRIM(MSG),'NTH = ', NML_SPEC%NTH
      WRITE (NDSN,12) TRIM(MSG),'FR1 = ', NML_SPEC%FR1
      WRITE (NDSN,12) TRIM(MSG),'TH1 = ', NML_SPEC%TH1
...
13  FORMAT (A,2X,A,L1)
  ...
  END SUBROUTINE REPORT_SPEC_NML
