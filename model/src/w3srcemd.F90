...
!$ACC END KERNELS
      ...
      WHERE(.NOT. SRC_MASK(1:NSEAC)) TWS(CHUNK0:CHUNKN) = 1./FMEANWS(1:NSEAC)
#endif
...
#ifdef W3_NL1
        IF (IQTPE.GT.0) THEN
          ! SNL1 refactored to process array of spectra:
          ! GPU Refactor: Note - passing DEPTH and WNMEAN into W3SNL1 separately now, not
          ! via temporary variable generated by DEPTH*WNMEAN.
          ! Avoids extra temporary array.
          CALL W3SNL1 ( SPEC(:,CHUNK0:CHUNKN), CG1_CHUNK(:,1:NSEAC), &
            WNMEAN(CHUNK0:CHUNKN), DEPTH(1:NSEAC), VSNL(:,1:NSEAC), VDNL(:,1:NSEAC), &
            SRC_MASK(1:NSEAC), NSEAC )
        ELSE
          DO CSEA=1,NSEAC
            IF(SRC_MASK(CSEA)) CYCLE
