...
!/    25-Feb-2021 : Updated for WW3 version 7.          ( version 7.12 )
!/
...
!     !/SMC   Enable Spherical Multiple-Cell (SMC) regridding.
!     !/RTD   Enable variable de-rotation.
!
! 10. Source code :
!
!/ ------------------------------------------------------------------- /
      USE CONSTANTS, ONLY: UNDEF, RADE
!/
...
!/
      ...
      USE W3ADATMD, ONLY: DW, UA, UD, AS, CX, CY, HS, WLM, T0M1, THM,  &
                          ...
                          WN, USSP, WBT
      USE W3ODATMD, ONLY: NDSO, NDSE, SCREEN, NDST, NOGRP, NGRPP,      &
                          IDOUT, FLOGRD, FNMPRE, NOSWLL, NOGE
!
      USE W3NMLMOPPMD
#ifdef W3_SMC
...
#ifdef W3_RTD
      ! Is pole really rotated?
      IF(POLAT < 90.0) FLRTD = .TRUE.
#endif
!
...
!
      INQUIRE(FILE=TRIM(FNMPRE)//"ww3_mopp.nml", EXIST=FLGNML) 
      IF (FLGNML) THEN
        COMSTR = "$"

        ! Read namelist
        CALL W3NMLMOPP (NDSI, TRIM(FNMPRE)//'ww3_mopp.nml', NML_FIELD, &
...
! 4.1 Time setup
        ...
        READ(NML_FIELD%TIMEREF, *)     DATETIME_TZERO
...
!
! 4.2 Output fields
        CALL W3FLGRDFLAG ( NDSO, SCREEN, NDSE, NML_FIELD%LIST, FLOG,   &
                           FLREQ, 1, 1, IERR )
        IF (IERR.NE.0) GOTO 800 

        MAPSTAOUT = NML_FIELD%MAPSTAOUT
...
! 4.4 PP specifics
        PP_LBEXP = NML_FIELD%PP_LBEXP
        PP_LBPROJ = NML_FIELD%PP_LBPROJ
...
!
        ...
        NOUT   = MAX ( 1 , NOUT )
  !
! CB        CALL STME21 ( TOUT , IDTIME )
! CB        WRITE (NDSO,940) IDTIME
  !
        TDUM = 0
!CB        CALL TICK21 ( TDUM , DTREQ )
!CB        CALL STME21 ( TDUM , IDTIME )
!CB        IF ( DTREQ .GE. 86400. ) THEN
!CB            WRITE (IDDDAY,'(I10,1X)') INT(DTREQ/86400.)
!CB          ELSE
!CB            IDDDAY = '           '
!CB          END IF
!CB        IDTIME(1:11) = IDDDAY
!CB        IDTIME(21:23) = '   '
!CB        WRITE (NDSO,941) IDTIME, NOUT
  !
  ! ... Output fields
  !
...
! Write inp/nml settings to screen:
...
!  
      IF( SMCGRD ) THEN
        ...
        WRITE(NDSO, 4100)
!
        ! For PP files, SMC output type is always 2 (re-gridded)
        SMCOTYPE = 2
        ...
        ! Set output grid definition to calculated SMC grid
        NXOUT = NXO
        NYOUT = NYO
        SXOUT = SXO
        SYOUT = SYO
...
!     TODO: updating for Met Office alternate partitioning.
      IF ( FLOG(4) ) THEN
        DO IPART=1, NBIPART
          IF ( TABIPART(IPART) .EQ. 0 ) THEN
            WRITE (NDSO,948)
          ELSE
            WRITE (NDSO,949) TABIPART(IPART)
          END IF
        ENDDO
      END IF
!
...
!
#ifdef W3_SMC
      DEALLOCATE(XIDX, YIDX, XSPAN, YSPAN, WTS, SMCIDX)
      DEALLOCATE(COV, MAPSMC)
#endif
!
! Formats
!
...
#ifdef W3_RTD
  3951 FORMAT ( '      Pole location    : ',2F9.3)
#endif
!
...
!
 ...
 4100 FORMAT (//'  SMC grid output :' /                               &
               ' --------------------------------------------------')
 4110 FORMAT ( '   SMC to regular lat/lon grid using area averaging' / &
...
!     Internal parameters
!     ----------------------------------------------------------------
!       XA      R.A.  Output fields
!      AUX1     R.A.  Auxiliary array for conversions
!     ----------------------------------------------------------------
!
!  4. Subroutines used :
!
...
!       !/SMC   Enable Spherical Multiple-Cell (SMC) regridding.
!       !/RTD   Enable variable de-rotation.
!       !/BT4   Bottom friction package (version 4).
!       !/IS2   Ice scattering term.
!
! 10. Source code :
!
!/ ------------------------------------------------------------------- /
      USE W3SERVMD, ONLY : W3S2XY, UV_TO_MAG_DIR, W3XYRTN, W3THRTN
...
!
      ...
      ALLOCATE(XA(NXOUT,NYOUT,MAX_COMP))
...
#ifdef W3_SMC
           !WRITE(NDSDAT) MAPSMC
           WRITE(NDSDAT) REAL(MAPSMC, KIND(4))
#endif
         ELSE
...
!
              IF( .NOT. VECTOR ) THEN
                CALL UV_TO_MAG_DIR(CX(1:NSEA), CY(1:NSEA), NSEA,       &
                                   TOLERANCE=0.05, CONV='N')
              ENDIF
!
              CALL S2GRID(CX(1:NSEA), XA(:,:,1))
              CALL S2GRID(CY(1:NSEA), XA(:,:,2))
              NFIELD=2
!
            ! Wind
...
!             
              ...
              ENDIF
!              
              CALL S2GRID(UA(1:NSEA), XA(:,:,1))
              CALL S2GRID(UD(1:NSEA), XA(:,:,2))
              NFIELD=2
!
...
!
#ifdef W3_BT4 ! Krumbein phi scale
           ELSE IF ( IFI .EQ. 1 .AND. IFJ .EQ. 8 ) THEN
             CALL S2GRID(SED_D50, XA(:,:,1))
             WHERE ( XA(:,:,1).NE.UNDEF) XA(:,:,1) = -LOG(XA(:,:,1)/0.001)/LOG2    
             NFIELD=1
#endif
!
#ifdef W3_IS2 ! Ice thickness
            ELSE IF (IFI .EQ. 1 .AND. IFJ .EQ. 9 ) THEN
              CALL S2GRID(ICEH(1:NSEA), XA(:,:,1))
              NFIELD=1
#endif
!
#ifdef W3_IS2 ! Maximum ice floe diameter
            ELSE IF (IFI .EQ. 1 .AND. IFJ .EQ. 10 ) THEN
              CALL S2GRID(ICEF(1:NSEA), XA(:,:,1))
              NFIELD=1
#endif
...
!CB#ifdef W3_RTD
!CB             IF(FLRTD) THEN
!CB               ! Rotate direction back to standard pole
!CB               IF ( FLAGUNR ) CALL W3THRTN(NSEA, TH1M(:,IK), AnglD, .FALSE.)
!CB             ENDIF
!CB#endif 
...
!CB              IF(FLRTD) THEN
!CB                ! Rotate direction back to standard pole
!CB                IF ( FLAGUNR ) CALL W3THRTN(NSEA, TH2M(:,IK), AnglD, .FALSE.)
!CB              ENDIF
!CB#endif
...
!              IF( SMCGRD ) THEN
!#ifdef W3_SMC
!                CALL W3S2XY_SMC( PDIR(:,IPART), XA, .TRUE. )
!#endif
!              ELSE
!                DO ISEA=1, NSEA
...
!              IF( SMCGRD ) THEN
!#ifdef W3_SMC
!                CALL W3S2XY_SMC( PTHP0(:,IPART), XA, .TRUE. )
!#endif
!              ELSE
!                DO ISEA=1, NSEA
...
!
            ...
            ELSE IF ( IFI .EQ. 5 .AND. IFJ .EQ. 1 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(UST(1:NSEA), XA(:,:,1))
              CALL S2GRID(USTDIR(1:NSEA), XA(:,:,2))
...
!
            ...
            ELSE IF ( IFI .EQ. 5 .AND. IFJ .EQ. 5 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(TAUWIX(1:NSEA), XA(:,:,1))
              CALL S2GRID(TAUWIY(1:NSEA), XA(:,:,2))
...
!
            ...
            ELSE IF ( IFI .EQ. 5 .AND. IFJ .EQ. 6 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(TAUWNX(1:NSEA), XA(:,:,1))
              CALL S2GRID(TAUWNY(1:NSEA), XA(:,:,2))
              NFIELD=2
!
            ! Whitecap coverage
...
!
            ...
            ELSE IF ( IFI .EQ. 6 .AND. IFJ .EQ. 2 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(TAUOX(1:NSEA), XA(:,:,1))
              CALL S2GRID(TAUOY(1:NSEA), XA(:,:,2))
              NFIELD=2
!
...
!
            ...
            ELSE IF ( IFI .EQ. 6 .AND. IFJ .EQ. 5 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(TUSX(1:NSEA), XA(:,:,1))
              CALL S2GRID(TUSY(1:NSEA), XA(:,:,2))
...
!
            ...
            ELSE IF ( IFI .EQ. 6 .AND. IFJ .EQ. 6 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(USSX(1:NSEA), XA(:,:,1))
              CALL S2GRID(USSY(1:NSEA), XA(:,:,2))
              !! Commented out unnecessary statements below for time being
              !! TAUWIX, TAUWIY are in north-east convention and XA,X2
              !! are not actually written out below
              !DO ISEA=1, NSEA
...
!
            ...
            ELSE IF ( IFI .EQ. 6 .AND. IFJ .EQ. 7 ) THEN
              NFIELD=2
              CALL S2GRID(PRMS(1:NSEA), XA(:,:,1))
              CALL S2GRID(TPMS(1:NSEA), XA(:,:,2))
!
...
!CB#ifdef W3_RTD
!CB              IF(FLRTD) THEN
!CB                ! Rotate x,y vector back to standard pole
!CB                IF ( FLAGUNR ) CALL W3XYRTN(NSEA, US3D(:,IK), US3D(:,NK+IK), AnglD)
!CB             ENDIF
!CB#endif
...
!
            ...
            ELSE IF ( IFI .EQ. 6 .AND. IFJ .EQ. 10 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(TAUICE(1:NSEA,1), XA(:,:,1))
              CALL S2GRID(TAUICE(1:NSEA,2), XA(:,:,2))
              NFIELD=2
!
...
!CB              IF(FLRTD) THEN
!CB                ! Rotate x,y vector back to standard pole
!CB                IF ( FLAGUNR ) CALL W3XYRTN(NSEA, USSP(:,IK), USSP(:,NK+IK), AnglD)
!CB              ENDIF
!CB#endif
...
!CB!
            ...
            ELSE IF ( IFI .EQ. 7 .AND. IFJ .EQ. 1 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(ABA(1:NSEA), XA(:,:,1))
              CALL S2GRID(ABD(1:NSEA), XA(:,:,2))
              NFIELD=2
!
            ...
            ELSE IF ( IFI .EQ. 7 .AND. IFJ .EQ. 2 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(UBA(1:NSEA), XA(:,:,1))
              CALL S2GRID(UBD(1:NSEA), XA(:,:,2))
              NFIELD=2
!
            ! Bottom roughness
...
!
            ...
            ELSE IF ( IFI .EQ. 7 .AND. IFJ .EQ. 5 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(TAUBBL(1:NSEA,1), XA(:,:,1))
              CALL S2GRID(TAUBBL(1:NSEA,2), XA(:,:,2))
              NFIELD=2
!
            ! Mean square slope
            ELSE IF ( IFI .EQ. 8 .AND. IFJ .EQ. 1 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(MSSX, XA(:,:,1))
              CALL S2GRID(MSSY, XA(:,:,2))
              NFIELD=2
!
            ! Phillips constant
            ELSE IF ( IFI .EQ. 8 .AND. IFJ .EQ. 2 ) THEN
              ...
              ENDIF
#endif
              CALL S2GRID(MSCX, XA(:,:,1))
              CALL S2GRID(MSCY, XA(:,:,2))
              NFIELD=2
!
            ! u direction for mss
...
!               ! Convert UNDEF to PP MDI
               IF(UNDEF .NE. PP_MDI) THEN
                 WHERE(XA(:,:,ICOMP) .EQ. UNDEF) XA(:,:,ICOMP) = PP_MDI
               ENDIF
               ...
               ELSE IF(PPIHEAD(21) .EQ. 1) THEN
                 ...
                 CALL CMPS_ALL(XA(:,:,ICOMP), DATACOMP, DATSZ, NXOUT, NYOUT,  &
                          SZCOMP, ISC, PP_MDI, IERR, CMESSAGE)
...
!/ ------------------------------------------------------------------- /
     ...
     ELSE ! IF(SMCGRD)
      ...
      IF(FLDR) THEN
        ...
        END DO
      ENDIF
      CALL W3S2XY2 ( NSEA, NSEA, NXOUT, NYOUT, S, MAPSF, X )
#ifdef W3_SMC
     ENDIF
#endif
